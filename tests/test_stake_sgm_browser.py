...
def test_ensure_sgm_fixture_ready_confirms_same_game_multi_tab():
    fixture_slug = "46575351-new-york-yankees-toronto-blue-jays"

    class FakeTabLocator:
        def __init__(self, page: "TabReadyPage", label: str) -> None:
            self.page = page
            self.label = label

        def count(self) -> int:
            return 1 if self.label == "Same Game Multi" else 0
        ...
        def click(self, *, timeout: int) -> None:
            self.page.clicked_tabs.append(self.label)

    class TabReadyPage(FakeReadyPage):
        def __init__(self, text: str) -> None:
            super().__init__(text)
        ...
        def get_by_text(self, label: str, *, exact: bool):
            ...
            return FakeTabLocator(self, label)

        def wait_for_timeout(self, ms: int) -> None:
            return None

    page = TabReadyPage("Main\nSame Game Multi\nPlayer Props")

    warnings = sgm_browser._ensure_sgm_fixture_ready(page, fixture_slug=fixture_slug)

    assert warnings == []
    assert page.clicked_tabs == ["Same Game Multi"]


def test_ensure_sgm_fixture_ready_retries_index_click_when_direct_page_misses_sgm(
    monkeypatch,
):
    fixture_slug = "46575351-new-york-yankees-toronto-blue-jays"
    page = FakeReadyPage("Main\nPlayer Props\nTeam/Match")
    index_clicks: list[str] = []

    def fake_open_fixture_from_index(page_arg, fixture_slug_arg: str):
        assert page_arg is page
        index_clicks.append(fixture_slug_arg)
        page.text = "Main\nSame Game Multi\nPlayer Props\nTeam/Match"
        return {"status": "clicked", "matchedBy": "team_names"}

    monkeypatch.setattr(
        sgm_browser,
        "_open_fixture_from_mlb_index",
        fake_open_fixture_from_index,
        raising=False,
    )

    warnings = sgm_browser._ensure_sgm_fixture_ready(
        page,
        fixture_slug=fixture_slug,
    )

    assert index_clicks == [fixture_slug]
    assert any("MLB index" in warning for warning in warnings)


def test_ensure_sgm_fixture_ready_does_not_retry_index_click_for_cloudflare(
    monkeypatch,
):
    page = FakeReadyPage(
        "Performing security verification. This website uses Cloudflare verification."
    )

    def fail_open_fixture_from_index(*args, **kwargs):
        raise AssertionError("index-click fallback should not run for access blockers")

    monkeypatch.setattr(
        sgm_browser,
        "_open_fixture_from_mlb_index",
        fail_open_fixture_from_index,
        raising=False,
    )

    with pytest.raises(RuntimeError, match="Cloudflare verification"):
        sgm_browser._ensure_sgm_fixture_ready(
            page,
            fixture_slug="46575351-new-york-yankees-toronto-blue-jays",
        )


def test_ensure_sgm_fixture_ready_can_start_with_index_click_navigation(monkeypatch):
    fixture_slug = "46575351-new-york-yankees-toronto-blue-jays"
    page = FakeReadyPage("Main\nPlayer Props\nTeam/Match")
    index_clicks: list[str] = []
    ...
    warnings = sgm_browser._ensure_sgm_fixture_ready(
        page,
        fixture_slug=fixture_slug,
        navigation_mode="index_click",
        fallback_to_index_click=False,
    )

    assert index_clicks == [fixture_slug]
    assert any("Stake MLB index" in warning for warning in warnings)


def test_open_fixture_from_mlb_index_clicks_matching_game_card(monkeypatch):
    fixture_slug = "46575351-new-york-yankees-toronto-blue-jays"

    class IndexClickPage:
        def __init__(self) -> None:
        ...
        def evaluate(self, script, arg=None):
            self.evaluate_args.append(arg or {})
            if "targetSlug" not in script:
                raise AssertionError("unexpected script")
            return {
                "status": "clicked",
                "matchedBy": "fixture_slug",
                "href": fixture_url(fixture_slug),
            }
    ...
    def fake_expand_mlb_game_list(page_arg, *, limit: int):
        assert page_arg is page
        expansion_calls.append(limit)
        return {"status": "expanded", "visibleGameCount": limit}

    monkeypatch.setattr(sgm_browser, "_expand_mlb_game_list", fake_expand_mlb_game_list)

    result = sgm_browser._open_fixture_from_mlb_index(page, fixture_slug)

    assert page.goto_calls == [sgm_browser.STAKE_MLB_URL]
    assert expansion_calls == [100]
    assert result["status"] == "clicked"
    assert result["matchedBy"] == "fixture_slug"
